...
from my_quant_config import AscendLinearMethod
...
class OpenPanguMLP(nn.Module):
    ...
    def __init__(
        ...
        # print(isinstance(quant_method, UnquantizedLinearMethod))
        # print(isinstance(quant_method, AscendLinearMethod))
        # print(isinstance(quant_method.quant_method, AscendW8A8DynamicLinearMethod))
        ...
        # === 新增：支持静态量化 (W8A8 Static) ===
        # 只要是 Ascend 的量化方法，且不是动态的，我们就默认为是静态 W8A8
        elif (isinstance(quant_method, AscendLinearMethod) and isinstance(
                quant_method.quant_method, AscendW8A8LinearMethod)):
            print("[INFO] 检测到静态量化配置 (W8A8 Static)，正在启用加速路径...")
            
            # 1. 激活函数配置
            # 使用 lambda 延迟获取，因为在 __init__ 时权重还没加载。
            # 静态模式下，weight_scale 依然存储在 layer.weight_scale (或 scales) 中
            # 注意：OpenPanguSiluAndMul 的实现需要支持只传入 Tensor 而不是 Tuple
            self.act_fn = OpenPanguSiluAndMul(
                weight_scale=lambda: self.gate_up_proj.weight_scale
            )
            
            # 2. Gate/Up Proj 配置
            # 目标：输入 Int8 -> 矩阵乘 -> 输出 Int32
            self.gate_up_proj._ascend_quant_config = {
                "output_dtype": torch.bfloat16,  # 必须是 Int32 以保留精度供 Silu 处理
                "pertoken_scale": False,      # [关键] 静态量化必须关闭这个！省去 ReduceSum 开销
                "return_scale": False,        # [关键] 不需要返回动态 Scale，因为我们有静态的 input_scale
            }
            
            # 3. Down Proj 配置
            # 目标：输入 Int8 (由 Silu 量化输出) -> 矩阵乘 -> 输出 BF16 (加回残差)
            self.down_proj._ascend_quant_config = {
                "output_dtype": torch.bfloat16, # 最终输出回 BF16/FP16
                "pertoken_scale": False,        # 关闭动态统计
                "return_scale": False,
            }
        ...
        # print("[INFO] OpenPanguMLP initialized.!!!!!!!!!!!!!!!!!!!!")
        # print(f"[INFO] OpenPanguMLP initialized with activation function: {type(self.act_fn)}")
...
class OpenPanguForCausalLM(nn.Module):
    ...
    def load_attn_mlp_weight(self,
        ...
        for (param_name, origin_name, shard_id) in attn_mlp_replace_mapping:
            ...
            # 量化特殊处理
            # skip = [".deq_scale", ".input_offset", ".quant_bias",".input_scale"]
            if (weight_name.endswith(".deq_scale") or   
                    weight_name.endswith(".quant_bias") or  
                    weight_name.endswith(".input_scale") or weight_name.endswith(".input_offset")) and weight_name not in params_dict:  
                    # 跳过缺失的量化参数  
                    continue  

            # print("weight_name:", weight_name)
            # print("(param_name, origin_name, shard_id):", (param_name, origin_name, shard_id))
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        for name, loaded_weight in weights:
            ...
            if self.load_attn_mlp_weight(attn_mlp_replace_mapping, params_dict, name, loaded_weight, loaded_params):
                ...
                # 量化特殊处理
                skip = [".deq_scale", ".input_offset", ".quant_bias",".input_scale"]
                if (name.endswith(".deq_scale") or   
                        name.endswith(".quant_bias") or  
                        name.endswith(".input_scale") or name.endswith(".input_offset")) and name not in params_dict:  
                        # 跳过缺失的量化参数  
                        continue  
